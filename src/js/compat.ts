...
export function loadBinaryFileSync(
  ...
  if (file_sub_resource_hash !== undefined) {
    ...
    if (hasher === undefined) {
      throw new Error(
        `unsupported subresource integrity hash algorithm ${algo}`,
      );
    }

    const hash = hasher(CryptoWordArray.create(response)).toString(
      CryptoBase64Encoding,
    );

    if (hash !== base64) {
      throw new Error(
        `subresource integrity mismatch for ${url}, expected ${file_sub_resource_hash} but found ${algo}-${hash}`,
      );
    }
  }
